...
describe("updateInterestGroupInformation", () => {
  ...
  test("does not exist", async () => {
    ...
    expect(await updateInterestGroupInformation(database.client, TestDatabase.userA.id, group)).toEqual(false);

    expect((await database.client`SELECT * FROM user_group`).length).toEqual(0);
    expect((await database.client` SELECT * FROM user_group_user_profile`).length).toEqual(0);
  });
  ...
  test("exists", async () => {
    ...
    expect(await createInterestGroup(database.client, TestDatabase.userA.id, original)).toEqual(true);
    expect(await updateInterestGroupInformation(database.client, TestDatabase.userA.id, mutated)).toEqual(true);
  ...
  test("invalid role", async () => {
    ...
    expect(await updateInterestGroupInformation(database.client, TestDatabase.userB.id, mutated)).toEqual(false);
    expect(await queryInterestGroups(database.client, 1)).toEqual([
      expect.objectContaining({
        ...original,
        members: "2",
